  ...
  describe("buildSwapData", function () {
    it("should build increase leverage swap data with exact-output", async function () {
      ...
      const mockAssemblyResponse = {
        gasEstimate: 500000,
        gasEstimateValue: 0.01,
        inputTokens: [{ tokenAddress: fixture.config.tokens.debt.address, amountDeducted: "7500000000000000000000" }],
        outputTokens: [{ tokenAddress: fixture.config.tokens.collateral.address, amountReceived: "5000000000000000000" }],
        netOutValue: -2500,
        outValues: [5000],
        transaction: {
          gas: 500000,
      ...
      const result = await swapDataBuilder.buildSwapData(quote, ethers.parseEther("5"), userAddress);
      ...
      expect(quoteRequest.outputTokens[0].amount).to.equal(ethers.parseEther("5").toString());
    ...
    it("should build decrease leverage swap data with flash fee included", async function () {
      ...
      const mockAssemblyResponse = {
        gasEstimate: 600000,
        gasEstimateValue: 0.015,
        inputTokens: [{ tokenAddress: fixture.config.tokens.collateral.address, amountDeducted: "2600000000000000000" }],
        outputTokens: [{ tokenAddress: fixture.config.tokens.debt.address, amountReceived: "5025000000000000000000" }],
        netOutValue: 2425,
        outValues: [5025],
        transaction: {
          gas: 600000,
      ...
      const userAddress = await fixture.deployer.getAddress();
      const result = await swapDataBuilder.buildSwapData(quote, ethers.parseEther("5000"), userAddress);
      ...
      expect(quoteRequest.outputTokens[0].amount).to.equal(totalDebtNeeded.toString());
    });
    ...
    it("should reject swaps with high price impact", async function () {
      ...
      await expect(
        swapDataBuilder.buildSwapData(quote, ethers.parseEther("5"), userAddress)
    ...
    it("should handle Odos router not configured", async function () {
      ...
      const swapDataBuilderNoOdos = new SwapDataBuilder(contractManager, configWithoutOdos);
      ...
      await expect(
        swapDataBuilderNoOdos.buildSwapData(quote, ethers.parseEther("5"), userAddress)
