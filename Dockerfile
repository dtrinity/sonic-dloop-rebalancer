...
RUN if [ -f ./scripts/replace_string.sh ] && [ -f /usr/src/cache/solidity-files-cache.json ]; then \
    sh ./scripts/replace_string.sh $HOST_PWD /usr/src /usr/src/cache/solidity-files-cache.json; \
    fi
